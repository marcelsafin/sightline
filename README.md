...
- Vitest (jsdom), 27 tests: content validation, WCAG contrast maths,
  structural selectors, the SEO and performance packs, the full engine workflow
  (`needs_input` → authored patch → approve → export → revert, AbortSignal),
  the approval state machine — including a regression test for concurrent
  `apply_fix` calls during the post-approval re-scan — and constitution checks (no fixture key is
